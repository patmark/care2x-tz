        ...
        if ($finalcommand == "delete") {
            if ($itemname != $linecount) {
                ...
                for ($i = $itemname + 1; $i <= $linecount; $i++) {
                    ...
                    if (!($db->Execute($sql)))
                        echo $sql . "  Vorbereitung der Daten zum L�schen ist gescheitert.";
                }
                // then delete the  data
                $sql = 'DELETE FROM ' . $dbtable . ' WHERE care_admission_patient_item="' . $itemname . '"';
                if (!($db->Execute($sql)))
                    echo $sql . "  Das L�schen der Daten ist gescheitert.";
            ...
            $linecount--;
            header("location: aufnahme_list.php?route=validroute&remark=itemdelete&batchnum=" . $batchnum . "&displaysize=" . $displaysize . "&linecount=" . $linecount . "&pagecount=" . $pagecount);
            exit;
        }else {
            ...
            if ($ergebnis)
                ...
                echo "Das Lesen von Daten aus der Datenbank ist gescheitert.";
        }
    };
}
else {
