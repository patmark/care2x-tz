...
<body <?php if ($rows) echo " bgcolor=#ffffee  onLoad=goactive() ";
    ...
    if (($mode != "") && ($saved)) {
        ...
        if ($inhalt['answered'])
            echo "	<b>$LDReply $LDFrom " . $inhalt['ansby'] . " " . $LDOn . " " . formatDate2Local($inhalt['astamp'], $date_format) . " " . convertTimeToLocal(formatDate2Local($inhalt['astamp'], $date_format, 0, 1)) . ":</b>";
        else
            echo "	<b>$LDYourReply :</b>";
        echo '	</td>
					</tr>
					<tr><td class="vn">';
        if (isset($mode) && ($mode == 'read')) {
            ...
            echo '<i>"' . nl2br($inhalt['reply']) . '"</i><br>
