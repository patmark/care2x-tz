...
if ($rows) {
    while ($content = $ergebnis->FetchRow()) {
        ...
        if (isset($content['answered']) && !empty($content['answered']))
            $sTemp = $sTemp . '<img ' . createComIcon($root_path, 'warn.gif', '0') . '>';
        $sTemp = $sTemp . '</a><p>';
    }
}
...
if ($inquirer)
    $smarty->assign('sInquirer', $inquirer);
elseif (isset($_SESSION['sess_user_name']))
    $smarty->assign('sInquirer', $_SESSION['sess_user_name']);
