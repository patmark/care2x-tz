...
<ul>
    ...
    <form name="berichtform" method="post" action="<?php echo $thisfile ?>" onSubmit="return pruf(this)">
		...
		<br><font size=1>' . $LDFrom . '</font> ';
                        if ($neff_date_range['fe_date'])
                            echo formatDate2Local($neff_date_range['fe_date'], $date_format);
                        echo ' <font size=1>' . $LDTo . '</font> ';
                        if ($neff_date_range['le_date'])
                            echo formatDate2Local($neff_date_range['le_date'], $date_format) . ' ';
                        ?>
                    </div></td></tr>
            <?php
