...
if ($enc_obj->loadEncounterData($pn)) {
    ...
    echo $enc_obj->getLastQuery() . "<p>$LDDbNoRead";
    exit;
}
...
if ($cfg['bname'] == 'msie') {
    ?>
            if (event.button == 2)
            {
                //alert("right click");
                if (winID == "dayback")
                    dayID = "<?php echo $LDStartDate ?>";
                if (winID == "dayfwd")
                    dayID = "<?php echo $LDEndDate ?>";
...
//****************************** DAy scale ********************************
    ...
    for ($i = $tag, $acttag = $tag, $d = 0, $tgbuf = $tagname; $i < ($tag + 7); $i++, $d++, $tgbuf++, $acttag++) {
        ...
        switch ($tgbuf) {
            case 0: echo' bgcolor="orange"';
                break;
            case 6: echo' bgcolor="#ffffcc"';
                break;
            case 7: echo' bgcolor="orange"';
                $tgbuf = 0;
                break;
        ...
        if (!$d)
            echo' align=left width="98">';else if ($d > 5)
            echo' align=right width="98">';
        else
            echo' align=center width="98">';
        if (!$d)
            echo '<a href="#">
...
//****************************** daily  diet plan ********************************
	...
	<font face="verdana,arial" size="2" color="#0" >';
        ...
        if ($r = getlatestdata($diet, $i, $kmonat, $jahr))
            echo hilite($r['short_notes']);
        else
            echo $LDDiet;
    }
//**************** Patient personal data ************************************
    ...
    echo '</td>
		...
		<div class=pcont><b>' . ucfirst($result['name_last']) . ', ' . ucfirst(ucwords($result['name_first'])) . '</b> <br>
...
//**************** curve graph ************************************
    ...
    echo '
		<img';
    if ($edit)
        echo ' ismap usemap="#FrontPageMap"';
...
//********************************** diagnose therapie daily report ****************************
    ...
    for ($i = $tag, $acttag = $tag, $d = 0; $i < ($tag + 7); $i++, $d++, $acttag++) {
        ...
        if ($edit)
            echo '<img src="' . $root_path . 'gui/img/common/default/pixel.gif" width="97" height="148"  border=0 alt="' . str_replace("~tagword~", $LDDiagnosisTherapy, $LDClk2EnterDaily) . '" >';
        if ($edit)
            echo "</a>";
        echo "
	</td>";
...
<font size=1 face="verdana,arial">';
    ...
    for ($i = $tag, $acttag = $tag, $d = 0; $i < ($tag + 7); $i++, $d++, $acttag++) {
        ...
        if ($r = &getdata($daily_etc, $i, $kmonat, $jahr))
            echo hilite($r);
...
// ************** anticoag dailydose ************************
	...
	<a href="javascript:popgetdailyinfo(\'anticoag_dailydose\',\'' . $pn . '\',\'' . $actjahr . '\',\'' . $actmonat . '\',\'' . $acttag . '\',\'' . ($d + $tagnamebuf) . '\',\'' . $jahr . '\',\'' . $kmonat . '\',\'' . $tag . '\',\'' . $tagname . '\')" title="' . str_replace("~tagword~", $LDAntiCoag, $LDClk2EnterDaily) . '" >';
        ...
        if ($edit)
            echo '<img src="p.gif" width="95" height="12"  align="absmiddle"  border=0 alt="' . str_replace("~tagword~", $LDAntiCoag, $LDClk2EnterDaily) . '" >';
        if ($edit)
            echo '</a>';
        echo '
	</td>';
...
// ************** medication ************************
    ...
    if ($edit)
        echo '<a href="javascript:popgetmedx(\'medication\',\'' . $pn . '\',\'' . $tag . '\')" title="' . str_replace("~tagword~", $LDMedication, $LDClk2Enter) . '">';
    echo $LDMedication;
    if ($edit)
        echo '</a>';
    ...
    for ($i = 0; $i < $maxmedx; $i++) {
        ...
        if ($m['article']) {
            switch ($m['color_marker']) {
                case "n": echo ' bgcolor="' . $bgc . '"';
                    $cat[$i] = "n";
                    break;
                case "a": echo ' bgcolor="#00ff00"';
                    $cat[$i] = "a";
                    break;
                case "w": echo ' bgcolor="#ffff00"';
                    $cat[$i] = "w";
                    break;
                case "c": echo ' bgcolor="#00ccff"';
                    $cat[$i] = "c";
                    break;
                case "i": echo ' bgcolor="#ff6699"';
                    $cat[$i] = "i";
                    break;
...
// ************** iv zugang dailydose ************************
	...
	<td valign="bottom" ';
        ...
        if ($edit)
            echo '<a href="javascript:popgetdailyinfo(\'iv_needle\',\'' . $pn . '\',\'' . $actjahr . '\',\'' . $actmonat . '\',\'' . $acttag . '\',\'' . ($d + $tagnamebuf) . '\',\'' . $jahr . '\',\'' . $kmonat . '\',\'' . $tag . '\',\'' . $tagname . '\')" title="' . str_replace("~tagword~", $LDIvPort, $LDClk2EnterDaily) . '">';
...
// ************** medication dailydose ************************
  ...
  <tr>
	...
	<table border="0" cellpadding="0"  cellspacing="1" width="100%">';
        if (is_object($medis))
            $medis->MoveFirst(); // reset medication object
        for ($j = 0; $j < $maxmedx; $j++) {
            if ($toggle)
                $bgc = "#efefef";
            else
            ...
            echo ' class="a10">&nbsp;';
            if ($edit)
                echo '<a href="javascript:popgetdailymedx(\'medication\',\'' . $pn . '\',\'' . $actjahr . '\',\'' . $actmonat . '\',\'' . $acttag . '\',\'' . ($d + $tagnamebuf) . '\',\'' . $jahr . '\',\'' . $kmonat . '\',\'' . $tag . '\',\'' . $tagname . '\')" title="' . str_replace("~tagword~", $LDMedication, $LDClk2PlanDaily) . '">';
