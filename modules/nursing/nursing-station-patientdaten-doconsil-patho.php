...
if ($pn == "")
    $smarty->assign('sOnLoadJs', $jsbuffer . ' document.searchform.searchkey.focus();"');
else
    $smarty->assign('sOnLoadJs', $jsbuffer . '"');
...
if ($mode == "edit" && $stored_request['quick_cut'])
    $tpbuffer.=' checked';
...
$tpbuffer = '';
if ($mode == "edit")
    $tpbuffer.=$stored_request['qc_phone'];
...
if ($mode == "edit" && $stored_request['quick_diagnosis'])
    $tpbuffer.=' checked';
...
$tpbuffer = '';
if ($mode == "edit")
    $tpbuffer.=$stored_request['qd_phone'];
...
if ($mode == "edit" && $stored_request['material_type'] == "pe")
    $tpbuffer.= "checked";
...
if ($mode == "edit" && $stored_request['material_type'] == "op_specimen")
    $tpbuffer.=" checked";
...
if ($mode == "edit" && $stored_request['material_type'] == "shave")
    $tpbuffer.= " checked";
...
if ($mode == "edit" && $stored_request['material_type'] == "cytology")
    $tpbuffer.= " checked";
$smarty->assign('input_material_type_cytology', $tpbuffer . '>');
$smarty->assign('LDCytology', $LDCytology);

if ($mode == "edit")
    $smarty->assign('val_material_desc', stripslashes($stored_request['material_desc']));

$smarty->assign('LDLocalization', $LDLocalization);
if ($mode == "edit")
    $smarty->assign('val_localization', stripslashes($stored_request['localization']));

$smarty->assign('LDClinicalQuestions', $LDClinicalQuestions);
if ($mode == "edit")
    $smarty->assign('val_clinical_note', stripslashes($stored_request['clinical_note']));

$smarty->assign('LDExtraInfo', $LDExtraInfo);
$smarty->assign('LDExtraInfoSample', $LDExtraInfoSample);
if ($mode == "edit")
    $smarty->assign('val_extra_note', stripslashes($stored_request['extra_note']));

$smarty->assign('LDRepeatedTest', $LDRepeatedTest);
$smarty->assign('LDRepeatedTestPls', $LDRepeatedTestPls);

if ($mode == "edit")
    $smarty->assign('val_repeat_note', stripslashes($stored_request['repeat_note']));
