...
//                alert("");
                var r = confirm("Are you sure you want to process bill without clearing balance? To clear balance,  \n\n\Please click Cancel and tick \"Clear Balance and Save Bill\" to process refund!");
                if (r === true) {
//                    x = "You pressed OK!";
                    return true;
                } else {
...
} else if ($bill_obj->GetAdvanceTotalAmountQuotation($_REQUEST['encounter_nr']) < 0) {
    echo 'document.getElementById(\'balance\').value=totalsum+' . abs($bill_obj->GetAdvanceTotalAmountQuotation($_REQUEST['encounter_nr'])) . '';
}
?>
                                    }
                                ...
                                <table width="600" border="0" align="center">
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            ...
                                            <input type="button" value ="<?php print $LDProcessAdvance; ?>" onclick="window.location.href = '<?php print $root_path . 'modules/billing_tz/billing_tz_advance.php' . URL_APPEND . '&batch_nr=' . $pid . '&encounter_nr=' . $_REQUEST['encounter_nr'] . '&bank_ref='.$bank_ref.'&ins_id='.$ins_id.'&patient=' . $patient; ?>';">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <script language="javascript">
                                    var total_sum;
                                    total_sum = sum_all();
                                    document.write(total_sum);</script>
                            </td>
                        </tr>
...
//                                } else { // need if !company_contract
                                ...
                                $insurance_list = $insurance_obj->ShowAllInsurancesForQuotatuion($ins_id);
...
if($bank_ref==1){
    ...
    if($ins_id==13){
        echo '<tr><td align="right" bgcolor="#BED0BB" class="style3">BANK REF</td><td bgcolor="#E2EAE1"><input type="text" name="bank" id="bank" placeholder="Enter Bank id here"></td></tr>';
        }
    }
?>
