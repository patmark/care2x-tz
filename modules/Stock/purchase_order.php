...
if ($addable) {
    ...
    while ($result3 = $purchaseorder->fetch_object($results)) {
        ...
        if ($num == 0) {
            ?><a href='purchase_order.php?del_order=$o&del_no=$noo'><IMG src='includes/images/delete.png' border='0'  alt='Delete'></a> <?php
                } echo"</td><td>$itemcode</td><td>$Quantity</td><td>$unit_cost</td><td>$totalcost</td><td><a href='purchase_order.php?order_id=$o&updateno=$noo&mode2=do&cost=$unit_cost&qty=$Quantity'>select</a></td></tr>";
            }// update form
            ?>
