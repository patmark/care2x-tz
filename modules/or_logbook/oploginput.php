...
if (!isset($thisday) || empty($thisday))
    $thisday = date('Y-m-d');
...
if ($mode == 'search' || $mode == 'paginate') {
    ...
    if (empty($GLOBAL_CONFIG['pagin_or_patient_search_max_block_rows']))
        $pagen->setMaxCount(MAX_BLOCK_ROWS);# Last resort, use the default defined at the start of this page
    else
        $pagen->setMaxCount($GLOBAL_CONFIG['pagin_or_patient_search_max_block_rows']);
                ...
                if ($ergebnis = $db->Execute($sql)) {
                    ...
                    if ($rows == 1) {
                        ...
                        if ($entry_time || $content['entry_out']) {
                            ...
                            if (trim($entry_time))
                                $dbuf[0] = strtr('s=' . $entry_time . '&e=' . $exit_time, ':', '.');
                            else
                        ...
                        if ($entry_time)
                            $eobuf = "s=$entry_time&e=$exit_time";
                        if ($cut_time)
                            $ccbuf = "s=$cut_time&e=$close_time";
...
if (!$_SESSION['sess_comdat'])
    $_SESSION['sess_comdat'];
# Set the user origin
...
switch ($mode) {
    ...
    case "fresh": echo 'resetlogdisplays();cleartimeframes();';
        break;
    case "chgdept": break;
    case "notimereset": break;
    case 'search': break;
    case 'edit': echo 'resettimebars();resettimeframe();';
        break;
    default: echo 'resetlogdisplays();';
}
if (!$patientfound)
    echo 'document.oppflegepatinfo.enc_nr.focus();';
?>
    ...
    <FORM METHOD="post" ACTION="oploginput.php?mode=save" name="oppflegepatinfo" onSubmit="return checksubmit()">
                                    ...
                                    <td>
                                        <table cellpadding=2 cellspacing=0 border=0 class="submenu">
                                            ...
                                            while ($pdata = $result->FetchRow()) {
                                                ...
                                                if ($sk && stristr($pdata['encounter_nr'], $sk))
                                                    echo '<u><b><span style="background:yellow"> ' . $pdata['encounter_nr'] . '</span></b></u>';
                                                else
                                                ...
                                                if ($sk && stristr($pdata['name_last'], $sk))
                                                    echo '<u><b><span style="background:yellow"> ' . $pdata['name_last'] . '</span></b></u>';
                                                else
                                                    echo $pdata['name_last'];
                                                echo '</a></td><td><FONT  SIZE=2>' . $ahref;
		...
		<input type="hidden" name="mode" value="save">';
                                                        ...
                                                        <TD valign="top" width=130><font  size=1  color="<?php if ($datafound)
                                                                echo "#0000cc";
                                                            else
                                                                echo "#3f3f3f";
                                                            ?>">
	...
	<font  size=1 color="#3f3f3f">';
                                                        ...
                                                        <TD valign=top width=150>
                                                            <font  size=1  color="<?php if ($datafound)
                                                                echo "#0000cc";
                                                            else
                                                                echo "#3f3f3f";
                                                            ?>">
    ...
    <?php echo "$LDAna<p>$LDAnaDoc" ?>
                                                            ...
                                                            <table cellpadding="0" cellspacing="0" border=0 width=100% class="v10_n"> 
                                                                <tr>
                                                                    ...
                                                                    <td><font  size=1 color="<?php if ($datafound)
                                                                                echo "#0000cc";
                                                                            else
                                                                                echo "#3f3f3f";
                                                                            ?>">
...
<?php if ($datafound) : ?>
	...
	<TEXTAREA NAME="op_therapy" COLS="18" ROWS="8"></TEXTAREA>';
                                                                                              ...
                                                                                              echo $LDTherapy . '/' . $LDOperation;
                                                                                          ?>
                                                        </TD>

                                                        <TD valign="top" width=140><font  size=1 color="<?php if ($datafound)
                                                                                              echo "#0000cc";
                                                                                          else
                                                                                              echo "#3f3f3f";
                                                                                          ?>"><?php echo $LDOpMainElements[result] ?><br>
<?php
if ($datafound)
    echo '
