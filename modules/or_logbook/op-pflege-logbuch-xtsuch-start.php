...
if (empty($srcword) && !empty($searchkey))
    $srcword = $searchkey;
    ...
    if ($mode == 'get' || $mode == 'getbypid' || $mode == 'getbyenc') {
        ...
        if ($ergebnis = $db->SelectLimit($sql, $pagen->MaxCount(), $pgx)) {
            if ($rows = $ergebnis->RecordCount()) {
                ...
                $select = "SELECT o.nr,o.op_nr,o.dept_nr,o.op_room,o.op_date, e.encounter_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex";
                $sql2 = " FROM
                ...
                if (is_numeric($srcword))
                    $sql2.=" OR  o.op_nr $sql_LIKE '$srcword%' OR e.encounter_nr $sql_LIKE '$srcword%'";
                if ($DOB)
                    $sql2.=" OR p.date_birth $sql_LIKE '$srcword%'";
                $sql2.=")";
...
if ((($mode == 'get') || ($datafound)) && $rows) {
    ...
    if ($rows) {
        ...
        while ($pdata = $ergebnis->FetchRow()) {
            ...
            if ($srcword && stristr($pdata['name_last'], $srcword))
                echo '<b><span style="background:yellow">' . $pdata['name_last'] . '</span></b>';
            else
            ...
            if ($srcword && stristr($pdata['name_first'], $srcword))
                echo '<b><span style="background:yellow">' . $pdata['name_first'] . '</span></b>';
            else
            ...
            if ($srcword && stristr($pdata['date_birth'], $srcword))
                echo '<b><span style="background:yellow">' . formatDate2Local($pdata['date_birth'], $date_format) . '</span></b>';
            else
