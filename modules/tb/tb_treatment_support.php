...
$filename = "tb_treatment_support.php";
...
$treatment_support_data = $o_tb_patient->getTreatmentSupportData();
...
if (isset($_POST['submit'])) {
    ...
    if (($o_val->getErrors()) == 0) {
        if ($_REQUEST['mode'] == 'edit') {
            if ($o_tb_patient->updateTBPatientTreatmentSupport($o_val->getValues())) {
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
            ...
            if ($o_tb_patient->insertTBPatientTreatmentSupport($o_val->getValues())) {
...
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
    ...
    if ($mode == 'edit') {
        $values = $o_tb_patient->getTreatmentSupportSID($_REQUEST['treatment_supporterid']);
    }
    if ($debug) {
        echo "<pre>";
        print_r($values);
