...
$filename = "tb_treatment_episodes.php";
...
if (!$o_tb_patient = new TB_patient($_REQUEST['pid'])) {
    require ("gui/gui_tb_treatment_episodes.php");
    die();
}
...
$treatment_episodes_data = $o_tb_patient->getTBTreatmentEpisodesData();
//print_r($treatment_episodes_data);exit;
...
//    exit;
    ...
    $o_val->set_rule('treatment_episode_start_date', 'rule_required');
    $o_val->set_rule('treatment_episode_regimen', 'rule_required');

    $o_val->set_rule('treatment_episode_end_date', 'rule_required');
    $o_val->set_rule('treatment_outcomeid', 'rule_required');
...
//    $o_val->set_rule('started_medication', 'rule_required');
    ...
    if (($o_val->getErrors()) == 0) {
        if ($_REQUEST['mode'] == 'edit') {
            if ($o_tb_patient->updateTBPatientTreatmentEpisode($o_val->getValues())) {
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
            ...
            if ($o_tb_patient->insertTBPatientTreatmentEpisode($o_val->getValues())) {
...
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
    ...
    if ($mode == 'edit') {
        $values = $o_tb_patient->getTBPatientTreatmentEpisode($_REQUEST['patient_treatment_episodeid']);
    }
    if ($debug) {
        echo "<pre>";
        print_r($values);
