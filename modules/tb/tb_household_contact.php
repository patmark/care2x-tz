...
$filename = "tb_household_contact.php";
...
$household_data = $o_tb_patient->getTBHouseholdData();
...
if (isset($_POST['submit'])) {
    ...
    $o_val->set_rule('household_contact_name', 'rule_required');
    $o_val->set_rule('household_contact_age', 'rule_required');
    $o_val->set_rule('household_contact_age', 'rule_numeric');
    $o_val->set_rule('household_contact_sex', 'rule_required');
    $o_val->set_rule('is_screened', 'rule_numeric');
//    $o_val->set_rule('outcome', 'rule_required');
//    $o_val->set_rule('started_medication', 'rule_required');
    ...
    if (($o_val->getErrors()) == 0) {
        if ($_REQUEST['mode'] == 'edit') {
            if ($o_tb_patient->updateTBPatientHouseholdContact($o_val->getValues())) {
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
            ...
            if ($o_tb_patient->insertTBPatientHouseholdContact($o_val->getValues())) {
...
//                header("Location: http://$host$uri/$filename" . URL_APPEND . "$add_breakfile");
    ...
    if ($mode == 'edit') {
        $values = $o_tb_patient->getTBPatientHouseholdContact($_REQUEST['patient_household_contactid']);
    }
    if ($debug) {
        echo "<pre>";
        print_r($values);
...
require ("gui/gui_tb_household_contact.php");
