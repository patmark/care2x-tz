...
for ($i = 0, $acttag = $day, $dyidx = $daytag - 1; $i < 21; $i++, $acttag++, $dyidx++) {
    ...
    if ($mday . $mmonth . $myear == $acttag . $month . $year)
        $spot = 1;
    echo ' 
    <td class="v18_b" ';
    if ($spot)
    ...
    if ($spot)
        ...
        echo 'cafenews-menu.php' . URL_APPEND . '&week=' . $week . '&myear=' . $year . '&mmonth=' . $month . '&mday=' . $acttag . '" ';
    echo ' title="' . formatDate2Local($year . '-' . $month . '-' . $acttag, $date_format) . '">';
    if ($spot)
        echo '<font color="#0000cc">';
    else
        echo '<font color="#d6d6d6">';
