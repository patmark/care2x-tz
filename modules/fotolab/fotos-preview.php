...
<body topmargin=0 marginheight=0>
	...
	if (file_exists($root_path . 'modules/fotolab/IJBasicViewer.class') && file_exists($root_path . 'modules/fotolab/ij.jar')) {
                ...
                if ($bShowImageJApplet)
                    echo $LDHideJavaApplet;
                else
                    echo $LDShowJavaApplet;
                echo '</a>&nbsp;</font>';
            }
            #
	...
	$iAppletHeight = $h + IMAGEJ_PANEL_HEIGHT;
                ...
                if ($toggle_pic)
                    echo '<a href="' . basename(__FILE__) . URL_APPEND . '&pn=' . $pn . '&nr=' . $nr . '&preview_size=' . $preview_size . '">';

                if ($t == 1) {
                    ?>

                    <img src="<?php echo $picsource; ?>" <?php if ($preview_size) echo 'width="' . $preview_size . '"';
             else echo $wh; ?> border=0  name="preview"
                         <?php
                     ...
                     if ($toggle_pic) {
                         ?>
                         alt="<?php if ($preview_size) echo $LDTogglePreviewOrig;
             else echo $LDToggleOrigPreview; ?>" 
                         title="<?php if ($preview_size) echo $LDTogglePreviewOrig;
             else echo $LDToggleOrigPreview; ?>"
                    <?php
                }
                ?>>
