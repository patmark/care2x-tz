...
# Body Onload js
if (!$same_pat)
    $smarty->assign('sOnLoadJs', 'onLoad="window.parent.PREVIEWFRAME.location.replace(\'fotolab-preview.php?sid=' . $sid . '&lang=' . $lang . '\');"');
else
    $smarty->assign('sOnLoadJs', 'onLoad="window.parent.PREVIEWFRAME.location.replace(\'fotolab-preview.php?sid=' . $sid . '&lang=' . $lang . '\');" ');
...
<table width=100% border=0 cellspacing=0 >
    ...
    <tr valign=top >
        <td colspan=2 style="border-bottom: 2px solid gray">
            ...
            echo '<a href="upload_search_patient.php' . URL_APPEND . '&target=search"><img ' . createLDImgSrc($root_path, $img, '0') . ' alt="' . $LDSearch . '" ';
            if ($cfg['dhtml'])
...
<?php echo $LDWantUpload ?> <input type="text" name="maxpic" size=1 maxlength=2 value="<?php echo $maxpic ?>"> <?php echo $LDImage ?> <input type="submit" value="<?php echo $LDGO ?>">
                ...
                <input type="hidden" name="firstname" value="<?php echo $firstname ?>">
                <input type="hidden" name="bday" value="<?php echo $bday ?>">
                <input type="hidden" name="lastnr" value="<?php $lastnr = 'nr' . ($maxpic - 1);
echo $$lastnr; ?>">
