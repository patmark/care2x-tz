...
<?php
if ($display == 'composite')
    echo 'topmargin=0 marginheight=0 leftmargin=0 marginwidth=0';
else
    echo 'topmargin=2 marginheight=2';
?>
    onLoad="if (window.focus)
                window.focus()" bgcolor=<?php echo $cfg['body_bgcolor']; ?>
...
<?php if ($display == 'composite') : ?>
            ...
            if (is_object($drg)) {
                ...
                if ($cat_obj = &$DRG_obj->ProcedureCategories())
                    $cat_ok = true;
                else
                    $cat_ok = false;
                        ...
                        if ($procedure['category_nr']) {
                            if ($cat_ok) {
                                while ($cat = $cat_obj->FetchRow()) {
                                    if ($procedure['category_nr'] == $cat['nr'])
                                        break;
                                }
                                if (defined('CATEGORY_NAME_FULL') && CATEGORY_NAME_FULL) {
                                    if (isset($$cat['LD_var']) && !empty($$cat['LD_var']))
                                        echo $$cat['LD_var'];
                                    else
                                        echo $cat['name'];
                                }else {
                                    if (isset($$cat['LD_var_short_code']) && !empty($$cat['LD_var_short_code']))
                                        echo $$cat['LD_var_short_code'];
                                    else
                                        echo $cat['short_code'];
                                }
                                # Reset the $cat_obj
		...
		';
			...
			<option value="' . $cat['nr'] . '"';
                        ...
                        if (defined('CATEGORY_NAME_FULL') && CATEGORY_NAME_FULL) {
                            if (isset($$procedure['cat_LD_var']) && !empty($$procedure['cat_LD_var']))
                                echo $$procedure['cat_LD_var'];
                            else
                                echo $procedure['cat_name'];
                        }else {
                            if (isset($$procedure['cat_LD_var_short_code']) && !empty($$procedure['cat_LD_var_short_code']))
                                echo $$procedure['cat_LD_var_short_code'];
                            else
                                echo $procedure['cat_short_code'];
                        }
                    }
                    echo '</td>
				<td><font size=2  color="' . $fcolor . '">';
                    if ($display != 'composite') {
                        if (defined('LOCALIZATION_NAME_FULL') && LOCALIZATION_NAME_FULL) {
                            if (isset($$procedure['loc_LD_var']) && !empty($$procedure['loc_LD_var']))
                                echo $$procedure['loc_LD_var'];
                            else
                                echo $icd['loc_name'];
                        }else {
                            if (isset($$procedure['loc_LD_var_short_code']) && !empty($$procedure['loc_LD_var_short_code']))
                                echo $$procedure['loc_LD_var_short_code'];
                            else
                                echo $procedure['loc_short_code'];
                        }
                    }elseif (!$edit) {
                        if ($procedure['localization']) {
                            if ($loc_ok) {
                                while ($loc = $loc_obj->FetchRow()) {
                                    if ($procedure['localization'] == $loc['nr'])
                                        break;
                                }
                                if (defined('LOCALIZATION_NAME_FULL') && LOCALIZATION_NAME_FULL) {
                                    if (isset($$loc['LD_var']) && !empty($$loc['LD_var']))
                                        echo $$loc['LD_var'];
                                    else
                                        echo $loc['name'];
                                }else {
                                    if (isset($$loc['LD_var_short_code']) && !empty($$loc['LD_var_short_code']))
                                        echo $$loc['LD_var_short_code'];
                                    else
                                        echo $loc['short_code'];
                                }
