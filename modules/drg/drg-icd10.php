		...
		<td><font size=2  color="' . $fcolor . '">';
			...
			';
					...
					<option value="' . $cat['nr'] . '"';
                        ...
                        if (defined('CATEGORY_NAME_FULL') && CATEGORY_NAME_FULL) {
                            if (isset($$icd['cat_LD_var']) && !empty($$icd['cat_LD_var']))
                                echo $$icd['cat_LD_var'];
                            else
                                echo $icd['cat_name'];
                        }else {
                            if (isset($$icd['cat_LD_var_short_code']) && !empty($$icd['cat_LD_var_short_code']))
                                echo $$icd['cat_LD_var_short_code'];
                            else
                                echo $icd['cat_short_code'];
                        }
                    }
                    echo '</td>
				<td><font size=2  color="' . $fcolor . '">';
                    ...
                    if ($display != 'composite') {
                        if (defined('LOCALIZATION_NAME_FULL') && LOCALIZATION_NAME_FULL) {
                            if (isset($$icd['loc_LD_var']) && !empty($$icd['loc_LD_var']))
                                echo $$icd['loc_LD_var'];
                            else
                                echo $icd['loc_name'];
                        }else {
                            if (isset($$icd['loc_LD_var_short_code']) && !empty($$icd['loc_LD_var_short_code']))
                                echo $$icd['loc_LD_var_short_code'];
                            else
                                echo $icd['loc_short_code'];
                        }
                    }elseif (!$edit) {
                        if ($icd['localization']) {
                            if ($loc_ok) {
                                while ($loc = $loc_obj->FetchRow()) {
                                    if ($icd['localization'] == $loc['nr'])
                                        break;
                                }
                                if (defined('LOCALIZATION_NAME_FULL') && LOCALIZATION_NAME_FULL) {
