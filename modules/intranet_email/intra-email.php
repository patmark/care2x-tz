        ...
        switch ($mode) {
                    ...
                    if ($ok && $db->CommitTrans()) {
                        ...
                        if ($ergebnis = $db->Execute($sql)) {
                            ...
                            if ($content[$folder] == '')
                                ...
                                $content[$folder].='_' . $buf;
                            if (empty($content['lastcheck']))
                                $content['lastcheck'] = '0001-01-01 00:00:00';
            ...
            case 'listmail': {
                    ...
                    if ($ergebnis = $db->Execute($sql)) {
                        if ($ergebnis->RecordCount()) {
                            ...
                            if ($folder == 'inbox') {
                                ...
                                $dbtable = 'care_mail_private';
                                if (empty($content['lastcheck']))
                                    $content['lastcheck'] = DBF_NODATETIME;
                                ...
                                if ($ergebnis = $db->Execute($sql)) {
                                    if ($ergebnis->RecordCount()) {
                                        ...
                                        while ($mails = $ergebnis->FetchRow()) {
                                            if (strlen($mails['subject']) > 30)
                                                $sub = substr($mails['subject'], 0, 30) . '...';
                                            else
                                                $sub = $mails['subject'];
                        ...
                        echo "$db_sqlquery_fail<p> $sql <p> $LDDbNoRead";
                    };
                    ...
                    if ($folder != 'inbox') {
                        ...
                        $newmails = 0;
                        $newmails = substr_count($content[inbox], 'r=0');
                    }
...
if ($mode == 'compose') {
    ...
    switch ($folder) {
        case 'inbox': $sTemp.=$LDInbox;
            break;
        case 'sent': $sTemp.=$LDSent;
            break;
        case 'drafts': $sTemp.=$LDDrafts;
            break;
        case 'trash': $sTemp.=$LDRecycle;
            break;
        default:
    }
}
...
if ($mode == 'compose')
    $smarty->assign('sOnLoadJs', 'onLoad="document.mailform.recipient.focus()"');
...
<?php if (($mode == 'compose') || ($mode == 'sendmail')) : ?>
        ...
        function save2draft()
        {
            ...
            if (d.subject.value == "")
                d.subject.value = "<?php echo $LDSubject ?>:";
            d.submit();
        }
        ...
        function chkCompose(d)
        {
            ...
            {
                if (confirm("<?php echo $LDAlertNoSubject ?>"))
                    return true;
                d.subject.focus();
                return false;
            }
        ...
        function useadd(a)
        {
            ...
            if (eval("document.mailform." + feld + ".value==''"))
                eval("document.mailform." + feld + ".value=a");
            else
                eval("document.mailform." + feld + ".value=document.mailform." + feld + ".value + '; '+a");
...
echo '
  &nbsp; <b>';
if ($mode != 'listmail')
    echo '<a href="intra-email.php' . URL_APPEND . '&mode=listmail">' . $LDInbox . '</a> | ';
else
    echo $LDInbox . ' | ';
if ($mode != 'compose')
    echo '<a href="intra-email.php' . URL_APPEND . '&mode=compose">' . $LDNewEmail . '</a> | ';
else
    echo $LDNewEmail . ' | ';
...
<table border=0 cellspacing=1 cellpadding=3 width="80%" height="80%">
    ...
    if ($sendok)
        ...
        echo '<input type="text" name="recipient" size=40 maxlength=40 value="' . $recipient . '" onFocus="feld=\'recipient\'">';
    echo '
          </td>
    ...
    <tr>
      ...
      <td  bgcolor="#f3f3f3">';
    if ($sendok)
        echo '<FONT face="Verdana,Helvetica,Arial" size=2 >' . $cc;
    else
        echo '<input type="text" name="cc" size=40 maxlength=40 value="' . $cc . '" onFocus="feld=\'cc\'">';
    echo '
	</td>
    </tr>
    <tr>
      ...
      <td  bgcolor="#f3f3f3">';
    if ($sendok)
        echo '<FONT face="Verdana,Helvetica,Arial" size=2 >' . $bcc;
    else
        echo '<input type="text" name="bcc" size=40 maxlength=40 value="' . $bcc . '" onFocus="feld=\'bcc\'">';
    echo '
	</td>
    </tr>
    <tr>
      ...
      <td  bgcolor="#f3f3f3">';
    if ($sendok)
        echo '<FONT face="Verdana,Helvetica,Arial" size=2 >' . $subject;
    else
        echo '<input type="text" name="subject" size=40 maxlength=150 value="' . $subject . '"  onFocus="feld=\'subject\'">';
    echo '
	</td>
    </tr>';
    /*   <tr>
    ...
    if (!$sendok) {
        ...
        echo '<FONT face="Verdana,Helvetica,Arial" size=2 >' . nl2br($body_txt);
    echo '
	    </td>
    </tr>
...
//******************************************* list mail *******************************
if ($mode == 'listmail') {  // prepare inbox for display
  ...
  <table border=0>
    ...
    if ($folder == 'inbox')
        ...
        echo '<a href="' . $thisfile . URL_APPEND . '&mode=listmail&l2h=' . $l2h . '"><img ' . $img_closefolder . '> ' . $LDInbox . '</a>';
    ...
    if ($folder == 'sent')
        ...
        echo '<a href="' . $thisfile . URL_APPEND . '&mode=listmail&l2h=' . $l2h . '&folder=sent"><img ' . $img_closefolder . '> ' . $LDSent . '</a>';
    echo '<font size=1 face=verdana,arial color="#0"> (';
    if ($folder == 'sent')
        echo $maxrow;
    else
    ...
    echo '<br>';
    if ($folder == 'drafts')
        echo '<img ' . $img_openfolder . '> <b>' . $LDDrafts . '</b>';
    else
        echo '<a href="' . $thisfile . URL_APPEND . '&mode=listmail&l2h=' . $l2h . '&folder=drafts"><img ' . $img_closefolder . '> ' . $LDDrafts . '</a>';
    echo '<font size=1 face=verdana,arial color="#0"> (';
    if ($folder == 'drafts')
        echo $maxrow;
    else
    ...
    echo '<br>';
    if ($folder == 'trash')
        echo '<img ' . $img_openfolder . '> <b>' . $LDRecycle . '</b>';
    else
        echo '<a href="' . $thisfile . URL_APPEND . '&mode=listmail&l2h=' . $l2h . '&folder=trash"><img ' . $img_closefolder . '> ' . $LDRecycle . '</a>';
    echo '<font size=1 face=verdana,arial color="#0"> (';
    if ($folder == 'trash')
        echo $maxrow;
    else
    ...
    if ($maxrow) {
        ...
        switch ($folder) {
            case 'inbox': echo "$LDInbox</b><br><img " . $img_closemail . "><font size=1 color=#0> " . str_replace('~nr~', $newmails, $LDEmailCount) . "</font>";
                break;
            case 'sent': echo $LDSent;
                break;
            case 'drafts': echo $LDDrafts;
                break;
            case 'trash': echo $LDRecycle;
                break;
        }
        echo '</font>
    ...
    <tr ><td colspan=6 height=1><img src="' . $root_path . 'gui/img/common/default/pixel.gif" border=0 height=4 width=1></td></tr>
	 <tr bgcolor="#0060ae">
       ...
       <td><FONT face="Verdana,Helvetica,Arial" size=2 color="#ffffff">&nbsp;';
        if ($l2h)
            echo '<a href="' . $thisfile . URL_APPEND . '&l2h=0&mode=listmail&folder=' . $folder . '" title="' . $LDSortDate . '"><img ' . $img_uparrow;
        else
            echo '<a href="' . $thisfile . URL_APPEND . '&l2h=1&mode=listmail&folder=' . $folder . '" title="' . $LDSortDate . '"><img ' . $img_dwnarrow;
        echo '
       ...
       <td><FONT face="Verdana,Helvetica,Arial" size=2 color="#ffffff">&nbsp;&nbsp;<b>' . $LDSize . ':</b>&nbsp;</td>
        ...
        for ($i = 0; $i < sizeof($arrlist); $i++) {
            ...
            echo '<td>&nbsp;';
            if ($minfo[r])
                echo '<a href="' . $buf . '"><img ' . $img_openmail . ' alt="' . $LDReadEmail . '"><br></a>';
            else
                echo '<img ' . $img_closemail . ' alt="' . $LDReadEmail . '"><br>';
  ...
  <input type="submit" value="' . $LDDelete . '">
	...
	';
        ...
        echo '<img ' . createMascot($root_path, 'mascot1_r.gif', '0', 'middle') . '>
			<FONT class="prompt">';
        switch ($folder) {
            case 'inbox': $fbuf = $LDInbox;
                break;
            case 'sent': $fbuf = $LDSent;
                break;
            case 'drafts': $fbuf = $LDDrafts;
                break;
            case 'trash': $fbuf = $LDRecycle;
                break;
        }
