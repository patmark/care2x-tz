...
if (($mode == 'saveadd') && ($addr == '')) {
    header("location:intra-email-addrbook.php" . URL_REDIRECT_APPEND);
    exit;
}
   ...
   function applyAddr()
        {
            ...
            for (i = 0; i < d.maxrow.value; i++)
            {
                ...
                if (v[0].checked)
                {
                    if (odm.recipient.value == "")
                        eval("odm.recipient.value=d.eaddr" + i + ".value");
                    else
                        eval("odm.recipient.value=odm.recipient.value+'; '+d.eaddr" + i + ".value");
                }
                if (v[1].checked)
                {
                    if (odm.cc.value == "")
                        eval("odm.cc.value=d.eaddr" + i + ".value");
                    else
                        eval("odm.cc.value=odm.cc.value+'; '+d.eaddr" + i + ".value");
                }
                if (v[2].checked)
                {
                    if (odm.bcc.value == "")
                        eval("odm.bcc.value=d.eaddr" + i + ".value");
                    else
                        eval("odm.bcc.value=odm.bcc.value+'; '+d.eaddr" + i + ".value");
                }
...
<BODY bgcolor=<?php echo $cfg['body_bgcolor']; ?> topmargin=0 leftmargin=0 marginwidth=0 marginheight=0 onLoad="if (window.focus)
            window.focus()" 
<?php if (!$cfg['dhtml']) {
...
// ******************************** show address book***************************************
    ...
    if ($l2h)
        echo '<a href="' . $thisfile . '' . URL_APPEND . '&l2h=0&mode=' . $mode . '&folder=' . $folder . '" title="' . $LDSortName . '"><img src="' . $root_path . 'gui/img/common/default/arw_down.gif" ';
    else
        echo '<a href="' . $thisfile . '' . URL_APPEND . '&l2h=1&mode=' . $mode . '&folder=' . $folder . '" title="' . $LDSortName . '"><img src="' . $root_path . 'gui/img/common/default/arw_up.gif" ';
    echo '
