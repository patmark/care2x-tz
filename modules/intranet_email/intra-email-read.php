  ...
  <input type="hidden" name="reply" value="0">';
    ...
    switch ($folder) {
        case 'inbox': if ($_COOKIE[$local_user . $sid] != $content['sender']) {
                $inp = 0;
                echo '<input type="button" value="' . $LDReply . '" onClick="submitForm(1)"> ';
                break;
            } else
                echo '<input type="button" value="' . $LDBack . '" onClick="window.location.href=\'intra-email.php' . URL_REDIRECT_APPEND . '&mode=listmail&folder=inbox&l2h=' . $l2h . '\'"> ';
            break;
        case 'sent': if ($_COOKIE[$local_user . $sid] != $content['recipient']) {
                $inp = 1;
                echo '<input type="button" value="' . $LDResend . '" onClick="submitForm(2)"> ';
                break;
            } else
                echo '<input type="button" value="' . $LDBack . '" onClick="window.location.href=\'intra-email.php' . URL_REDIRECT_APPEND . '&mode=listmail&folder=sent&l2h=' . $l2h . '\'"> ';
            break;
        case 'drafts':$inp = 1;
            echo '
 								 <input type="button" value="' . $LDNewEmail . '" onClick="submitForm(2)"> ';
            break;
        case 'trash':if ($_COOKIE[$local_user . $sid] != $content['recipient']) {
                ...
                $inp = 0;
                echo '  <input type="button" value="' . $LDReplyAgain . '" onClick="submitForm(1)"> ';
            } break;
    }
