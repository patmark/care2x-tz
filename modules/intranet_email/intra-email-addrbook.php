    ...
    if ($rows = $ergebnis->RecordCount()) {
        ...
        switch ($task) {
            ...
            case 'saveadd': {
                    ...
                    if ($content['addr_book'] == "")
                        $content['addr_book'] = $buf;
                    else
                        $content[addr_book].="_" . $buf;
            ...
            case 'delete': {  //$content[addr_book]=strtolower($content[addr_book]);
...
if ($task == "newadd")
    $smarty->assign('sOnLoadJs', 'onLoad="document.newform.name.focus();"');
    ...
    function chkDelete(d, m)
    {
        for (i = 0; i < m; i++) {
            if (eval("d.del" + i + ".checked"))
                if (confirm("<?php echo $LDConfirmDeleteAddr ?>"))
                    return true;
                else {
                    for (i = 0; i < m; i++)
                        if (eval("d.del" + i + ".checked"))
                            eval("d.del" + i + ".checked=false");
