...
if (!$_SESSION['nhif_auth_no']) { //Authorization number assigned to nhif patients
    $_SESSION['nhif_auth_no'];
}

if (!$_SESSION['nhif_visit_type']) { //Visit type assigned to nhif patients
    $_SESSION['nhif_visit_type'];
}

if (!$_SESSION['nhif_referral_no']) { //Referral number assigned to nhif patients
    $_SESSION['nhif_referral_no'];
}
        ...
        if (($mode == 'save') || ($forcesave != '')) {
            ...
            if (!$error) {
                ...
                if ($update || $encounter_nr) {
                    ...
                    //Check here if encounter has nhif authorization code and add to encounter table
                    if (isset($_SESSION['nhif_auth_no']) && $_SESSION['nhif_auth_no'] != '') {
                        $_POST['nhif_auth_no'] = $_SESSION['nhif_auth_no'];
                    }

                    $_POST['encounter_type'] = $_SESSION['nhif_visit_type'];
                    $_POST['referral_no'] = $_SESSION['nhif_referral_no'];
...
//                    print_r($_POST);
                    ...
                    if ($encounter_obj->insertDataFromInternalArray()) {
                        ...
                        if (isset($_SESSION['nhif_auth_no'])) {
                            //Unset 
                            unset($_SESSION['nhif_auth_no']);
                        }
                        if (isset($_SESSION['nhif_visit_type'])) {
                            //Unset 
                            unset($_SESSION['nhif_visit_type']);
                        }
                        if (isset($_SESSION['nhif_referral_no'])) {
                            //Unset 
                            unset($_SESSION['nhif_referral_no']);
                        }
                        if ($is_transmit_to_weberp_enable == 1) {
...
//							else $sTemp = $sTemp.$result['name'];
    ...
    $vTemp .= '<input name="encounter_type" type="radio" value="3" ';
