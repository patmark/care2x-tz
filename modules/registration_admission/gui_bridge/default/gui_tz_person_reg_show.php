...
<script type="text/javascript">
    ...
    function verify_card(cardno) {
        var visit = '';
        var selectedVisit = $("input[type='radio'][name='encounter_type']:checked");
        var ReferralNo = $("input[type='text'][name='referralNo']").val();

//        alert(selectedVisit.val());
//        return false;
        if (selectedVisit.val() > 0) {
            visit = selectedVisit.val();
        } else {
            alert('Please select visit type!');
            return false;
        }
        //Check if selected visit type is referral and require referral number
        if (selectedVisit.val() == 3) {
            if (ReferralNo == "") {
                alert('Please fill the referral number field!');
                return false;
            }
        }
        ProgressCreate(10);
        $(document).ready(function () {
            $.ajax({
                url: "<?php echo $root_path; ?>modules/nhif/authorize.php",
                type: 'GET',
                data: {card_no: cardno,
                    visit_type: visit,
    ...
    function verify_visit() {
        ...
        $(document).ready(function () {
            $.ajax({
                url: "<?php echo $root_path; ?>modules/registration_admission/visit_type.php",
                type: 'GET',
...
//                        echo 'Auth: ' . $_SESSION['nhif_auth_no'];
                        ?>
