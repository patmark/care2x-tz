...
if ($mode == 'search') {
    ...
    if (($keyword == '') || ($keyword == '%') || ($keyword == '_')) {
        header("location:$thisfile" . URL_REDIRECT_APPEND . "&invalid=1&cat=$cat&userck=$userck");
        exit;
    }
    if ($lang == 'de') {
        if (eregi($keyword, 'eilig'))
            $keyword = 'urgent';
    }
    if (!$ofset)
        $ofset = 0;
...
if ((($mode == 'search') || $update) && ($keyword != '')) {
    ...
    if ($such_date) {
        switch (strtolower($date_format)) {
            case 'yyyy-mm-dd' : $separator = '-';
                break;
            case 'mm/dd/yyyy' : $separator = '/';
                break;
    ...
    if ($ergebnis = $db->SelectLimit($sql, $nrows, $ofset))
        $linecount = $ergebnis->RecordCount();   //count rows=linecount
...
//reset result
    if (!$linecount) {
        ...
        $sql = "SELECT o.* FROM $dbtable AS o  LEFT JOIN care_department AS d ON o.dept_nr=d.nr
									WHERE (";
        if ($sdt)
            $sql = $sql . "o.order_date $sql_LIKE '$sdt'
												OR";
    ...
    if ($linecount > 0) {
        ...
        while ($content = $ergebnis->FetchRow()) {
            ...
            if (isset($$buffer) && !empty($$buffer))
                ...
                echo $dept[$content['dept_nr']]['name_formal'];
            ...
            if ($content['status'] == 'normal')
                echo '&nbsp;';
            else if ($content['priority'] == 'urgent')
                echo '<img ' . createComIcon($root_path, 'warn.gif', '0') . ' alt="' . $LDUrgent . '!">';
