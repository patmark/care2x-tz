...
<?php
switch ($mode) {
    case "add":echo ' onLoad="location.replace(\'#bottom\')"   ';
        break;
    case "delete":echo ' onLoad="location.replace(\'#' . ($idx - 1) . '\')"   ';
        break;
    case "send": if ($sendok)
            echo ' onLoad="hide_bcat()" ';
}
...
?>>
    ...
    if ($error) {
        ...
        <table border=0>
            <tr>
                ...
                <td><font face="Verdana, Arial" size=3 color="#800000"><b><?php echo ($error == 'password') ? $LDInvalidPassword : $LDUnknownValidator;
    echo '<br>' . $LDPlsEnterInfo; ?></b></font></td>
            </tr>
        </table>
        <hr>
        <?php
    ...
    if ($cat == 'pharma')
        ...
        $dbtable = $dbtable = 'care_med_orderlist';
...
# ++++++++++++++++++++++++ show the actual list +++++++++++++++++++++++++++
    ...
    if ($rows > 0) {
        ...
        if (($mode != 'send') && (!$sendok)) {
            ...
            if (!isset($prior) || $prior == 'normal' || $prior == '')
                echo ' checked';
            echo '> 
			' . $LDUrgent . '<input type="radio" name="prior" value="urgent" ';

            if (isset($prior) && $prior == 'urgent')
                echo ' checked';
