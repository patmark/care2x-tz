...
if ($_REQUEST['change'] == true) {
    $sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['rest_weberp_income_enable'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'rest_weberp_income_enable\' LIMIT 1;';
    $db->Execute($sql);
}

$glob_obj->getConfig('rest_weberp_income_enable');
if ($GLOBAL_CONFIG['rest_weberp_income_enable'] == "") {
    $sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
            . 'VALUES (\'rest_weberp_income_enable\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
    $db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">REST Service for WebERP Income Interface Configuration</font>
                                </td>
                        ...
                        <form  method="post" name="form">
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Enable the REST Income Interface to WebERP?'; ?></b></FONT></td>
                                    <td  align="left">

                                        <select name="rest_weberp_income_enable">
                                            <?php
                                            if ($GLOBAL_CONFIG['rest_weberp_income_enable'] == 1) {
