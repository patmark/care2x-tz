...
if ($_REQUEST['change'] == true) {
     $sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['icd_code_type'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'icd_code_type\' LIMIT 1;';
    $db->Execute($sql);
}

$glob_obj->getConfig('icd_code_type');
if ($GLOBAL_CONFIG['icd_code_type'] == "") {
    $sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
            . 'VALUES (\'icd_code_type\', \'10\', \'ICD code to be used for diagnosis\', \'\', \'\', \'\', NOW(), \'\', NOW());';
    $db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">ICD Codes Type For Diagnosis </font>
                                </td>
                                <td bgcolor="#99ccff" align=right>
                                    <a  href="edv.php?sid=<?php echo $sid . "&lang=" . $lang; ?>73&ntid=false"><img src="../../gui/img/control/blue_aqua/en/en_back2.gif" border=0 width="76" height="21" alt="" style="filter:alpha(opacity=70)" onMouseover="hilite(this, 1)" onMouseOut="hilite(this, 0)" ></a><a
                        ...
                        <form  method="post" name="form">
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Select ICD Codes For Diagnosis:'; ?></b></FONT></td>
                                    <td  align="left">

                                        <select name="icd_code_type">
                                            <?php
                                            if ($GLOBAL_CONFIG['icd_code_type'] == 9) {
                                                echo '<option value="9" selected>ICD 9</option>';
                                                echo '<option value="10">ICD 10</option>';
                                            } else {
                                                echo '<option value="9">ICD 9</option>';
                                                echo '<option value="10" selected>ICD 10</option>';
                                            }
                                            ?>
                                        </select>
