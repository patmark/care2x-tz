...
class DHTML_Calendar {
    ...
    function DHTML_Calendar($calendar_lib_path = 'jscalendar', $lang = 'en', $theme = 'calendar-win2k-1', $stripped = true) {
        if ($stripped) {
    ...
    function show_calendar($calendar, $date_format, $input_field, $value = '') {
        ...
        $calendarJS = $calendar->make_input_field(
            ...
            'button' => 'trigger'), array('name' => $input_field,
            'value' => $value,
            'style' => 'width:90px;'), $date_format);
        return $calendarJS;
    }
    ...
    function make_input_field($cal_options = array(), $field_attributes = array(), $date_format) {
        $code = '';
        $id = $this->_gen_id();
        $attrstr = $this->_make_html_attr(array_merge($field_attributes, array('id' => $this->_field_id($id),
            'type' => 'text')));
