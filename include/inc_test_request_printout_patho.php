...
if ($printmode) {
    if ($stored_request['entry_date'] != DBF_NODATE)
    ...
    if ($stored_request['status'] == 'pending') {
        ...
        if ($stored_request['entry_date'] && $stored_request['entry_date'] != DBF_NODATE)
            $tpbuffer.= @formatDate2Local($stored_request['entry_date'], $date_format) . '" onBlur="IsValidDate(this,\'' . $date_format . '\')';
        else
            $tpbuffer.= @formatDate2Local(date('Y-m-d'), $date_format);
...
if ($stored_request['quick_cut'])
    $smarty->assign('input_quick_cut', '<img ' . createComIcon($root_path, 'chkbox_chk.gif', '0', 'absmiddle'));
else
    $smarty->assign('input_quick_cut', '<img ' . createComIcon($root_path, 'chkbox_blk.gif', '0', 'absmiddle'));
...
if ($stored_request['quick_diagnosis'])
    $smarty->assign('input_quick_diagnosis', '<img ' . createComIcon($root_path, 'chkbox_chk.gif', '0', 'absmiddle') . '>');
else
    $smarty->assign('input_quick_diagnosis', '<img ' . createComIcon($root_path, 'chkbox_blk.gif', '0', 'absmiddle') . '>');
...
if ($stored_request['material_type'] == "pe")
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_chk.gif', '0', 'absmiddle');
else
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_blk.gif', '0', 'absmiddle');
$smarty->assign('input_material_type_pe', $tpbuffer . '>');

$smarty->assign('LDPE', $LDPE);

if ($stored_request['material_type'] == "op_specimen")
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_chk.gif', '0', 'absmiddle');
else
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_blk.gif', '0', 'absmiddle');
...
if ($stored_request['material_type'] == "shave")
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_chk.gif', '0', 'absmiddle');
else
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_blk.gif', '0', 'absmiddle');
...
if ($stored_request['material_type'] == "cytology")
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_chk.gif', '0', 'absmiddle');
else
    $tpbuffer = '<img ' . createComIcon($root_path, 'radio_blk.gif', '0', 'absmiddle');
