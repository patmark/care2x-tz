...
class Diagnostics extends Encounter {
    ...
    var $tbl_diagnosis_pract = 'practitioner_nr';
    ...
    /*
     * Constructor function to check the type of icd and
     * assign variables respectively
     */

    function Diagnostics($icd_code_type = 10) {
        if ($icd_code_type == 10) {  //Names for ICD 10
            $this->tbl = "care_icd10_en";
            $this->tbl_groups = "care_tz_icd10_quicklist";
        } else if ($icd_code_type == 9) {
            $this->tbl = "care_icd9_en";
            $this->tbl_groups = "care_tz_icd9_quicklist";
        }
    }
        ...
        while (list($x, $v) = each($dataarray)) {
            if (strstr($x, "diagnosistype")) {
                ...
                if ($dataarray['diagnosistype_' . $form_id] == "revisit")
                    $return = $this->insert_diagnosis_as_revisit($dataarray['parentcase_' . $form_id], $dataarray['pid'], $dataarray['encounter'], $dataarray['icd_' . $form_id], $dataarray['icddesc_' . $form_id], $dataarray['comment_' . $form_id], $dataarray['diagnosistype_' . $form_id], $dataarray['doctor_' . $form_id], $dataarray['practitioner_nr']);
                else
                    $return = $this->insert_diagnosis_as_new($dataarray['pid'], $dataarray['encounter'], $dataarray['icd_' . $form_id], $dataarray['icddesc_' . $form_id], $dataarray['comment_' . $form_id], $dataarray['diagnosistype_' . $form_id], $dataarray['doctor_' . $form_id], $dataarray['practitioner_nr']);
            }
            if ($return)
    ...
    function insert_diagnosis_as_new($pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, $type, $doctor, $pract_no) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($debug)
    ...
    function insert_diagnosis_as_revisit($parent_case_nr, $pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, $type, $doctor, $pract_no) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
