...
class Product extends Core {
    ...
    function get_array_search_results($keyword, $result_filter) {
        ...
        if ($keyword == "*") {
            ...
            $this->sql = "SELECT `item_id`, `partcode`, `item_description`,`not_in_use` FROM " . $this->tbl_product_items . " WHERE `item_description` LIKE '%" . $keyword . "%'";
...
//this selects items from specific cateogry
    ...
    /*
     * A function to return NHIF Item Code
     * get_nhif_item_code($item_id)
     */

    function get_nhif_item_code($item_id) {
        global $db;
