...
class dental extends Core {
    ...
    function GetPatientNameFromPid($pid) {
        ...
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->row = $this->result->FetchRow()) {
                return ucfirst($this->row['name_last']) . ' ' . ucwords($this->row['name_first']);
            } else {
                return false;
            }
        } else {
...
//        $db->debug=true;
        ...
        $this->sql = "SELECT date, notes, create_id, short_notes, encounter_nr, type_nr, nr,create_time FROM care_encounter_notes WHERE (encounter_nr IN (SELECT encounter_nr FROM care_encounter WHERE pid = '$pid')) " . $MORE . " ORDER BY create_time ASC";
