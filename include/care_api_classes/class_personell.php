...
class Personell extends Core {
    ...
    /**
     * Table name for personell roles
     * @var string
     */
    var $tb_care_role_person = 'care_role_person';
    ...
    function getDoctorsOfDept($dept_nr = 0) {
        if (!$dept_nr)
            ...
            return $this->_getAllPersonell(1, 17, $dept_nr); // 1= dept (location), 17 = doctor (role)
    }
    ...
    function getNursesOfDept($dept_nr = 0) {
        if (!$dept_nr)
            ...
            return $this->_getAllPersonell(1, 16, $dept_nr); // 1= dept (location); 16 = nurse (role)
    }
    ...
    function _getAllPersonell($loc_type_nr, $role_nr = 0, $dept_nr) {
        ...
        $sql = "SELECT a.nr, a.personell_nr, crp.name AS job_function_title, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
				FROM 	$this->tb_assign AS a,
							$this->tb AS ps,
							$this->tb_person AS p,
                                                        $this->tb_care_role_person AS crp
				WHERE a.role_nr = $role_nr
					...
					AND ps.pid=p.pid 
                                        AND ps.job_function_title=crp.nr
    ...
    function _getAllPersonellAllDepts($loc_type_nr, $role_nr = 0) {
        ...
        $sql = "SELECT a.nr, a.personell_nr, crp.name AS job_function_title, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
				...
				WHERE a.role_nr=$role_nr
					...
					AND ps.pid=p.pid
                                        AND ps.job_function_title=crp.nr
				ORDER BY p.name_last as name_last, p.name_first";
    ...
    function searchPersonellBasicInfo($key, $oitem = 'name_last', $odir = 'ASC', $limit = FALSE, $len = 30, $so = 0) {
        ...
        if (empty($key))
            return FALSE;
        $this->sql = "SELECT ps.nr, j.name AS job_function_title, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
    ...
    function loadPersonellData($nr = 0) {
        ...
        $this->sql = "SELECT ps.nr, ps.practitionerno, ps.pid, ps.job_type_nr, j.name AS job_function_title, ps.date_join, ps.date_exit,
    ...
    /*
     * function userExists
     * Checks if an employee already exists as
     * a user and return a username if found
     */

    function userExists($personell_nr = 0) {
        global $db;
        if (!$personell_nr) {
            return FALSE;
        } else {
            $sql = "SELECT login_id, personell_nr FROM $this->tbl_users WHERE personell_nr=$personell_nr";
            if ($this->result = $db->Execute($sql)) {
                if ($this->result->RecordCount()) {
                    $this->row = $this->result->FetchRow();
                    return $this->row['login_id'];
                } else {
                    return FALSE;
                }
            } else {
