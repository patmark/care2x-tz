...
class Ward extends Encounter {
    ...
    /**
     * Returns ward pharmacy based on its record number.
     * @access public
     * @param int Record number
     * @return mixed string or boolean
     */
    function WardPharmacy($nr) {
        global $db;
        if (empty($nr)) {
            return false;
        }
        if ($this->result = $db->Execute("SELECT pharmacy FROM $this->tb_ward WHERE nr=$nr")) {
            if ($this->result->RecordCount()) {
                $this->row = $this->result->FetchRow();
                return $this->row['pharmacy'];
            } else {
                return false;
            }
        } else {
