...
class weberp {
    ...
    var $weberpcalls = array(
        ...
        'getLocationExists' => "weberp.xmlrpc_GetLocationExists",
...
//        $this->ServerURL = $weberp_base;
//
//        $this->user = php_xmlrpc_encode($weberp_api_user);
//        $this->password = php_xmlrpc_encode($weberp_api_pass);
    ...
    function get_location_exists_in_webERP($Location) {
        $adjustmentData[0] = $Location;

        $transmit = $this->transfer($adjustmentData, $this->weberpcalls['getLocationExists']);
        if ($transmit[0] == 0) {
            return $transmit[1];
        } else {
            return false;
