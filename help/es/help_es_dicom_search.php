<font face="Verdana, Arial" size=3 color="#0000cc">
<b>
    Radiolog�a - B�squeda en las im�genes DICOM

</b>
</font>
<p><font size=2 face="verdana,arial" >
<form action="#" >
    <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img <?php echo createComIcon('../', 'frage.gif', '0') ?>> 
    <font color="#990000"><b> �C�mo buscar la imagen DICOM de un paciente?</b> </font></font> 
    <ul>
        <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Paso 1: </b>Escriba 
        ya sea la informaci�n completa, los primeros digitos del numero de paciente, 
        apellido , o nombre en el campo de busqueda correspondiente. <br>
        <b>Paso 2: </b>D� clic al boton <img <?php echo createLDImgSrc('../', 'searchlamp.gif', '0') ?>> 
        para iniciar la busqueda de imagenes. </font> 
        <p> 
        <ul>
            <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Nota: </b>Usted 
            tambi�n puede usar los comodines % o * </font> 
            <p> <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Nota: 
                </b>Si la b�squeda tiene resultado positivo, aparecer� una lista de im�genes. 
                </font>
            <p> 
        </ul>
    </ul>
    <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img <?php echo createComIcon('../', 'frage.gif', '0') ?>> 
    <font color="#990000"><b> �C�mo mostrar la imagen o im�genes?</b> </font></font> 
    <ul>
        <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Paso: </b> 
        D� clic al �cono <img <?php echo createComIcon('../', 'torso.gif', '0', 'absmiddle') ?>> 
        para mostrar la imagen dentro de la ventana actual (espec�fico para el visualizador).</font> 
        <p> <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img src="../help/es/img/es_dicom_in.png" border=0>
            </font>
    </ul>
    <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img <?php echo createComIcon('../', 'frage.gif', '0') ?>> 
    <font color="#990000"><b> �C�mo muestro las im�genes en una ventana nueva?</b> 
    </font></font> 
    <ul>
        <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Paso: </b> 
        D� clic al �cono <img <?php echo createComIcon('../', 'torso_win.gif', '0', 'absmiddle') ?>> 
        para mostrar las im�genes en una ventana nueva.</font> 
        <p> <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img src="../help/es/img/es_dicom_ex.png" border=0>
            </font>
    </ul>
    <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><img <?php echo createComIcon('../', 'warn.gif', '0', 'absmiddle') ?>> 
    <font color="#990000"><b> Nota:</b></font></font> 
    <ul>
        <font size="2" face="Verdana, Arial, Helvetica, sans-serif"> Si usted decide 
        cerrar d� clic al bot�n <img <?php echo createLDImgSrc('../', 'close2.gif', '0') ?>>. 
        </font> 
    </ul>



</form>

