...
function validarea($area, $zeile2, $range) {
    for ($i = 0; $i < $range; $i++)
        if (($zeile2[$i] == $area)or ( $zeile2[$i] == "alle"))
            return 1;
    return 0;
}
...
if (($username != "")or ( $keyword != '') && ($passtag)) {
    ...
    switch ($passtag) {
        case 1:$errbuf = $errbuf . "Falsche Eingabe";
            echo 'Sorry, aber Ihre Eingaben sind falsch. Versuchen Sie es noch ein mal.';
            break;
        case 2:$errbuf = $errbuf . "Keine Berechtigung";
            echo 'Sie sind nicht berechtigt in den Bereich zu gehen!';
            break;
        default:$errbuf = $errbuf . "Zugang gesperrt";
            echo 'Ihre Zugangsberechtigung ist gesperrt. Setzen Sie sich bitte mit der EDV in Verbindung.';
    }
