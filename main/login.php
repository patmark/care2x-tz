...
if ((($pass == 'check') && ($keyword != '')) && ($userid != '')) {
    ...
    if ($user->isKnown() && $user->hasValidPassword()) {
        if ($user->isNotLocked()) {
            ...
            $_SESSION['sess_practitionerno'] = $user->practitionerno();
            ...
            //Get the ICD Code in use and assign to session
            require_once ($root_path . 'include/care_api_classes/class_globalconfig.php');
            $glob_obj = new GlobalConfig;

            $icd_code_type = $glob_obj->getConfigValue($type = 'icd_code_type');
            if ($icd_code_type == '' || is_null($icd_code_type)) {
                $icd_code_type = 10;   //Set default ICD 10 
            }
            $_SESSION['sess_icd_code_type'] = $icd_code_type;
            
            //Get facility code and assign to session
            $_SESSION['facility_code'] = $glob_obj->getConfigValue($type = 'main_info_facility_code');
