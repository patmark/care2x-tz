...
for ($i = 0, $j = 1; $i < sizeof($element); $i++, $j++) {
    ...
    for ($n = 0; $n < sizeof($datarray); $n++) {
        ...
        if (($buff < $buff2) || ($dat['e'] == NULL)) {
            if (($dat['s'] != NULL) && ($dat['e'] != NULL))
                ImageFilledRectangle($im, ($buff * $tabcols), ($tabrows * (0.65 + $j)), ($buff2 * $tabcols), ($tabrows * (0.85 + $j)), $red);
            if (($dat['s'] != NULL) && ($dat['e'] == NULL))
                ImageFilledRectangle($im, ($buff * $tabcols), ($tabrows * (0.65 + $j)), (($buff * $tabcols) + 4), ($tabrows * (0.85 + $j)), $red);
            if (($dat['s'] == NULL) && ($dat['e'] != NULL))
                ImageFilledRectangle($im, (($buff2 * $tabcols) + 4), ($tabrows * (0.65 + $j)), ($buff2 * $tabcols), ($tabrows * (0.85 + $j)), $red);
        }
        else {
