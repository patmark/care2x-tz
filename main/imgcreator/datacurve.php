...
if (!isset($mo) || empty($mo))
    $mo = date('m');
if (!isset($dy) || empty($dy))
    $dy = date('d');
if (!isset($yr) || empty($yr))
    $yr = date('Y');
...
#**************** begin of curve tracing  Blood Pressure***************
        if ($bprows) {
            for ($i = 0; $i < $bprows; $i++) {
                ...
                if ($bp['msr_date'] != $date)
                    continue;
                if (empty($bp['msr_time']) || empty($bp['value']))
                    continue;
                $ox2 = (($bp['msr_time']) * $xunit) + $xof;
                $oy2 = (($bp['value']) - 70) * $yunit_bp;
                $oy2 = 134 - $oy2;
                ImageArc($im, $ox2, $oy2, 4, 4, 0, 360, $text_red);
                if ($ox1 || $oy1)
                    ImageLine($im, $ox1, $oy1, $ox2, $oy2, $text_red);
                $ox1 = $ox2;
                $oy1 = $oy2;
            }
            $bp_obj->MoveFirst();
...
#**************** begin of curve tracing  Temperature***************
        if ($trows) {
            for ($i = 0; $i < $trows; $i++) {
                ...
                if ($bc['msr_date'] != $date)
                    continue;
                if (empty($bc['msr_time']) || empty($bc['value']))
                    continue;
                $tx2 = (($bc['msr_time']) * $xunit) + $xof;
                $ty2 = (($bc['value']) - 35) * $yunit_temp;
                $ty2 = 134 - $ty2;
                ImageFilledRectangle($im, $tx2 - 2, $ty2 - 2, $tx2 + 1, $ty2 + 1, $text_blue);
                if ($tx1 || $ty1)
                    ImageLine($im, $tx1, $ty1, $tx2, $ty2, $text_blue);
                $tx1 = $tx2;
                $ty1 = $ty2;
            }
            $temp_obj->MoveFirst();
