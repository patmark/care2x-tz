...
for ($n = 0, $j = 95; $n < 3; $n++, $j+=95)
    ImageLine($im, $j, 0, $j, 25, $black);
for ($n = 0, $j = 114; $n < 4; $n++, $j+=114)
    ImageLine($im, $j, 334, $j, 359, $black);
ImageLine($im, 285, 26, 285, 220, $black);
ImageLine($im, 285, 50, 569, 50, $black);
ImageLine($im, 285, 75, 569, 75, $black);
for ($n = 0, $j = 380; $n < 2; $n++, $j+=95)
    ImageLine($im, $j, 0, $j, 50, $black);
...
if ($ttf_render) {
    ...
    ImageTTFText($im, 12, 0, 10, 80, $black, $arial, ar2uni($result['name_last']) . ', ' . ar2uni(ucwords($result['name_first'])));
    ...
    ImageString($im, 3, 10, 70, $result['name_last'] . ', ' . ucwords($result['name_first']), $black);
...
if ($result['payer_other'] == '') {
    if ($ttf_render) {
        ImageTTFText($im, 12, 0, 10, 170, $black, $arial, ar2uni($result['name_last']) . ', ' . ar2uni(ucwords($result['name_first'])));
        ...
        ImageString($im, 3, 10, 160, $result['name_last'] . ', ' . ucwords($result['name_first']), $black);
...
if ($ttf_ok) {
    ...
    ImageTTFText($label, 16, 0, $lmargin, $tmargin + 56, $eblack, $arial, ar2uni($result['name_last']) . ', ' . ar2uni(ucwords($result['name_first'])));
    # Date of birth
    ...
    ImageString($label, 5, 10, 40, $result['name_last'] . ', ' . ucwords($result['name_first']), $black);
...
if ($ttf_ok) {
    ...
    ImageTTFText($label2, 11, 0, 10, 75, $black, $arial, ar2uni(ucwords($result['name_first'])));
    ...
    ImageString($label2, 4, 10, 65, ucwords($result['name_first']), $black);
...
if ($ttf_ok) {
    ...
    ImageTTFText($label3, 12, 0, 10, 49, $black, $arial, ar2uni(ucwords($result['name_first'])));
    ...
    ImageString($label3, 4, 10, 40, ucwords($result['name_first']), $black);
...
for ($i = 0, $j = 1; $i < 1080; $i+=135, $j++) {
    if ($j > 4)
        ...
        ImageCopy($im, $label3, 570, $i + 5, 0, 0, 173, 133);
    if (defined('DRAW_BORDER') && DRAW_BORDER)
