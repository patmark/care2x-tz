...
<?php
if ($mode == "ex")
    echo "url='excolorchg.php?sid=$sid&lang=$lang&item=$item&mode=change&uid=$r&color='+c;";
else
    echo "url='colorchg.php?sid=$sid&lang=$lang&item=$item&mode=change&uid=$r&color='+c;";
?>
...
<?php echo $LDPlsClkColor ?><br></font>
    ...
    <ul>
        <a href="#" <?php if ($d) echo 'onClick="opener.focus();window.close()"';
else echo 'onClick="window.close()"'; ?>><img <?php echo createLDImgSrc($root_path, 'cancel.gif', '0') ?>  alt="<?php echo $LDCancel ?>"></a>
    </ul>
    ...
    if (file_exists($root_path . 'language/' . $lang . '/' . $lang . '_copyrite.php'))
        ...
        include('../language/en/en_copyrite.php');
    ?>
    </FONT>
